...
public class NewSave {
	...
	public String[] strings = {"Strength: ", "Charisma: ", "Wisdom: ", "Intelligence: ", "Dexterity: ", "Constitution: "};
	...
	private void initialize() {
		...
		btnCreate.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				GraphicMain.get_Handler().getG().getFm().Write(GraphicMain.get_Handler().getG().getP().stats);
				//add stat array
				frmNewSave.dispose();
			}
		});
	...
	public void Create(String text) {
		...
		refresh();
		String[] stats = {text, Integer.toString(values[0]), Integer.toString(values[1]), Integer.toString(values[2]), Integer.toString(values[3]),
				Integer.toString(values[4]), Integer.toString(values[5])};
		for(int i = 0; i < 7; i++) {
			System.out.println(stats[i]);
		}
		System.out.println("done");
		//GraphicMain.get_Handler().getG().getP().Create(text);
		GraphicMain.get_Handler().getG().getP().initStats(stats);
		GraphicMain.get_Handler().getG().Begin();
		return;
	}
